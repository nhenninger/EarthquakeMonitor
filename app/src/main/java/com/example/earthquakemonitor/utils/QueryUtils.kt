...
object QueryUtils {
    ...
    fun extractEarthquakes(): ArrayList<Earthquake> {
        ...
        try {
            val features = JSONObject(SAMPLE_JSON_RESPONSE).optJSONArray("features")
            for (i in 0 until (features?.length() ?: 0)){   // JSONArray lacks .indices or .withIndex()
                val properties = features.getJSONObject(i).getJSONObject("properties")
                val magnitude = properties.optDouble("mag")
                val place = properties.optString("place")
                val date = properties.getLong("time")
                earthquakes.add(Earthquake(magnitude.toString(), place, date))
            }
